...
//作为default_pmm_manager的成员函数，在page_init里对memmap里的每个map
static void
...
default_alloc_pages(size_t n) {
	...
	while (le != &free_list) {
		//通过le遍历链表，寻找一个作为连续空闲块起始页且连续页数大于等于n
		//访问property的方式需要借助offset
		struct Page *p = le2page(le, page_link);

		//找到足够长度
		if (p->property >= n) {
			//需要补充的：此处>=n，可能会有剩余的页，首先把n个调整状态并摘掉，剩下的保留在链表上
			int iter = 0;
...
default_check(void) {
	int count = 0, total = 0;
	list_entry_t *le = &free_list;
	//遍历了链表，检测是否每个page的property位都置1表示可用
	while ((le = list_next(le)) != &free_list) {
		struct Page *p = le2page(le, page_link);
