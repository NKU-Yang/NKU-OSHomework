...
default_init_memmap(struct Page *base, size_t n) {
    ...
    list_add_before(&free_list, &(base->page_link));//将p插入空闲页的链表里面
}

static struct Page *
default_alloc_pages(size_t n) {
    assert(n > 0);
    if (n > nr_free) {//如果总的空闲物理页数<n则返回NULL
        return NULL;
    }
    //遍历空闲链表
    ...
    while ((le = list_next(le)) != &free_list) {
        // 此处le2page就是将le的地址-page_link在Page的偏移从而找到Page的地址
        struct Page *p = le2page(le, page_link);
        if (p->property >= n) {//如果找到大于等于n的空闲块则停止
            page = p;
            break;
        }
    }
    if (page != NULL) {
        if (page->property > n) {//如果空闲快大小大于n，则对块进行分割，将剩余块加入链表，删除原来的空闲快
            struct Page *p = page + n;//分割后剩余块的Page地址
...
default_free_pages(struct Page *base, size_t n) {
    assert(n > 0);
    //遍历链表，检查需要释放的物理页是否真的被占用
    struct Page *p = base;
    for (; p != base + n; p ++) {
        assert(!PageReserved(p) && !PageProperty(p));
        p->flags = 0;//设置标记为
        set_page_ref(p, 0);//应用计数清零
    }
    //将新的空闲快插入到链表中
    base->property = n;//设置空闲块大小
    SetPageProperty(base);//设置标志位
    list_entry_t *le = list_next(&free_list);
    //遍历链表，检查base是否有相邻空闲块，有的话与base合并，并删除相邻的节点
    while (le != &free_list) {
        ...
        if (base + base->property == p) {
            ...
            ClearPageProperty(p);//标志位清0
            list_del(&(p->page_link));//删除节点
        }
        //base与之前空闲块相邻
